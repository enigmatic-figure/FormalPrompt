...
Compilation is deterministic and permitted only after structural validation, semantic readiness, and explicit approval. It first claims the approved revision as `compiling`, stages generic handoff files and any typed initialization artifacts inside the run directory, then publishes the result before making the run terminal. It never modifies the caller's project automatically.
...
- Compilation requires an approval record tied to the current document revision.

## Context isolation

The caller supplies the initial document and eventually receives a compact result object. Facilitator transcripts, review deliberation, browser events, and intermediate revisions remain in the run directory and external agent processes. A handoff contains the final execution brief, artifact manifest, unresolved-item count, approval revision, and paths—not the full transcript.

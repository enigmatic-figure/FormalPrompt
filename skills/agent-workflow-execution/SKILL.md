...
4. Determine readiness from incoming ports and recorded evidence. Run only nodes whose required inputs are satisfied. Respect maximum_parallel_nodes.
...
6. For an operation, review, or gate, require the evidence named by the node. Never infer a pass from process exit, file presence, or a model's unsupported claim.
